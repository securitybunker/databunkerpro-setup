...
DATABUNKER="https://localhost:3000"

RESULT=`curl -k -s $DATABUNKER/v1/pactivity/share-data-with-sms-provider -XPOST \
...
RESULT=`curl -k -s $DATABUNKER/v1/lbasis/send-sms -XPOST \
   -H "X-Bunker-Token: $XTOKEN"  -H "Content-Type: application/json" \
...
RESULT=`curl -k -s $DATABUNKER/v1/pactivity/share-data-with-sms-provider/blah -XPOST \
   -H "X-Bunker-Token: $XTOKEN"`
echo "Trying to link a fake legal basis to processing activity: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/pactivity/share-data-with-sms-provider/core-send-sms-on-login -XPOST \
   -H "X-Bunker-Token: $XTOKEN"`
...
RESULT=`curl -k -s $DATABUNKER/v1/pactivity/share-data-with-sms-provider/core-send-email-on-login -XPOST \
   -H "X-Bunker-Token: $XTOKEN"`
echo "Linking existing legal basis 3 to processing activity: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/pactivity/share-data-with-sms-provider/send-sms -XPOST \
   -H "X-Bunker-Token: $XTOKEN"`
echo "Linking existing legal basis to processing activity: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/pactivity/share-data-with-sms-provider/send-sms -XDELETE \
   -H "X-Bunker-Token: $XTOKEN"`
echo "Unlinking legal basis to processing activity: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/pactivity -H "X-Bunker-Token: $XTOKEN"`
echo "Get a list of processing activities: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/lbasis -H "X-Bunker-Token: $XTOKEN"`
echo "Get a list of legal basis objects: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/lbasis/send-sms -XDELETE -H "X-Bunker-Token: $XTOKEN"`
echo "Deleting legal basis object: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/core-send-sms-on-login/email/test@securitybunker.io -XPOST \
   -H "X-Bunker-Token: $XTOKEN"  -H "Content-Type: application/json" \
   -d '{"lawfulbasis":"contract"}'`
echo "Giving consent for legal basis obj 2: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/core-send-sms-on-login -XDELETE -H "X-Bunker-Token: $XTOKEN"`
echo "Revoking legal basis object 2: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/contract-approval/email/test@securitybunker.io -XPOST \
   -H "X-Bunker-Token: $XTOKEN"  -H "Content-Type: application/json" \
   -d '{"lawfulbasis":"contract"}'`
echo "Giving consent for a fake legal basis: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/core-send-sms-on-login/email/test@securitybunker.io -XDELETE -H "X-Bunker-Token: $XTOKEN"`
echo "Withdraw legal basis 2 consent: $RESULT"

echo "Creating user."
RESULT=`curl -k -s $DATABUNKER/v1/user \
  -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
...
if [ "$STATUS" = "error" ]; then
  ...
  RESULT=`curl -k -s -X PUT $DATABUNKER/v1/user/phone/4444 \
	  -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
          -d '{"fname":"Test","lname":"Account","email":"test@securitybunker.io"}'`
  echo "Result: $RESULT"
  RESULT=`curl -k -s -X PUT $DATABUNKER/v1/user/email/test@securitybunker.io \
          -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
...
RESULT=`curl -k -s $DATABUNKER/v1/userapp/token/$TOKEN/shipping \
  -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
...
RESULT=`curl -k -s $DATABUNKER/v1/userapp/token/$TOKEN/shipping -XPUT \
  -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
  -d '{"status":"delivered"}'`
...
RESULT=`curl -k -s $DATABUNKER/v1/sharedrecord/token/$TOKEN \
  -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
...
RESULT=`curl -k -s $DATABUNKER/v1/get/$RECORD`
echo "Get shared record (no password/access token): $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/userapp/token/$TOKEN \
   -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json"`
echo "View list of all user apps $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/userapps \
   -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json"`
echo "View list of all apps $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/send-sms/token/$TOKEN -XPOST \
...
RESULT=`curl -k -s $DATABUNKER/v1/agreement/core-send-sms-on-login/phone/4444 -XDELETE \
   -H "X-Bunker-Token: $XTOKEN"`
echo "Withdraw consent send-sms for user by phone: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/send-sms/token/$TOKEN \
   -H "X-Bunker-Token: $XTOKEN"`
echo "View this specific consent only: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/token/$TOKEN \
   -H "X-Bunker-Token: $XTOKEN"`
echo "View all user consents: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/agreement/send-sms \
   -H "X-Bunker-Token: $XTOKEN"`
echo "View all users with send-sms consent on: $RESULT"

SESSION="84493d20-2ea5-11eb-86f0-2e04ce962524"

RESULT=`curl -k -s $DATABUNKER/v1/session/$SESSION -XPOST \
   -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json" \
   -d '{"clientip":"1.1.1.1","x-forwarded-for":"2.2.2.2","info":"email"}'`
echo "Create session 2: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/session/$SESSION \
   -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json"`
echo "Get session 1: $RESULT"

RESULT=`curl -k -s $DATABUNKER/v1/sessions/phone/4444 \
   -H "X-Bunker-Token: $XTOKEN" -H "Content-Type: application/json"`
...
RESULT=`curl -k -s $DATABUNKER/v1/user/custom/alex -H "X-Bunker-Token: $XTOKEN"`
echo "Result: $RESULT"
